...
# ==============================
# Pydantic Models (修复 422 错误的核心)
# ==============================
class ApplicantProfile(BaseModel):
    # 全部设为可选，防止前端本地缓存了旧格式数据导致校验失败
    name: str = "Unknown"
...
class ProfInfo(BaseModel):
    ...
    # 关键修复：允许 List[str] 或 str。爬虫通常返回 List。
...
<head>
    ...
    <style>
        ...
        .btn {{ padding: 10px 18px; background: var(--primary); color: white; border: none; border-radius: 6px; cursor: pointer; font-weight: 600; transition: 0.2s; display: inline-flex; align-items: center; gap: 8px; text-decoration: none; font-size: 0.9rem; user-select: none; }}
...
<body>
    ...
    <script>
        ...
        function renderTargets() {{
            ...
            // 重要：同步数据到 Map，确保列表页的弹窗也能工作
            targets.forEach(p => {{
                window.profDataMap[p.id_hash] = p;
            }});
...
async def dashboard():
    ...
    <div style="background:white; padding:30px; border-radius:12px; margin-bottom:30px;">
        ...
        <form method="POST" action="/scrape" style="display:flex; gap:10px;">
            <input type="text" name="uni" class="form-input" placeholder="输入学校名称 (例如: HKU, MIT)" required>
...
async def uni_detail(uni_name: str):
    ...
    extra_js = f"<script>{''.join(js_data_map_lines)}</script>"
...
async def generate_email(request: GenerateEmailRequest):
    try:
        ...
        # 数据清洗：将 Pydantic 兼容的 Union 类型统一转为字符串给 Agent

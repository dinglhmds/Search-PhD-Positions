...
class ProfInfo(BaseModel):
    ...
    link: Optional[str] = None  # 新增 Link 字段支持
...
def get_base_layout(content: str, title: str = "PhD-App", active_tab: str = "search", extra_js: str = "") -> str:
    return f"""
<!DOCTYPE html>
<html lang="zh-CN">
<head>
...
<body>
    <div class="sidebar">
        <div class="logo"><i class="fas fa-graduation-cap"></i> PhD-App</div>
...
async def dashboard():
    ...
    <div style="background:white; padding:30px; border-radius:12px; margin-bottom:30px;">
        ...
        <form method="POST" action="/scrape" style="display:flex; gap:10px;">
            <input type="text" name="uni" class="form-input" style="flex:1;" placeholder="输入学校名称 (例如: UMD Caltech)" required>
...
async def uni_detail(uni_name: str):
    ...
    for prof in faculty_list:
        ...
        prof_link = prof.get('link', '')
        
        if prof_link and prof_link != "未提供" and prof_link.startswith("http"):
            name_html = f'''
                <a href="{prof_link}" target="_blank" style="color:#0f172a; text-decoration:none; border-bottom: 1px dotted #0f172a; transition:0.2s;" onmouseover="this.style.color='var(--primary)';this.style.borderBottomColor='var(--primary)'" onmouseout="this.style.color='#0f172a';this.style.borderBottomColor='#0f172a'">
                    {prof_name} <i class="fas fa-external-link-alt" style="font-size:0.7em; opacity:0.6; margin-left:4px;"></i>
                </a>
            '''
        else:
            name_html = prof_name
        ...
        <div class="card search-item" data-search="{search_text}">
            <div class="card-header">
                <h3 class="card-title">{name_html}</h3>
